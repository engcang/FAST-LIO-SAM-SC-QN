...
void FastLioSamScQnClass::updateVisVars(const PosePcd &pose_pcd_in)
{
...
visualization_msgs::Marker FastLioSamScQnClass::getLoopMarkers(const gtsam::Values &corrected_esti_in)
{
...
void FastLioSamScQnClass::voxelizePcd(pcl::VoxelGrid<PointType> &voxelgrid, pcl::PointCloud<PointType> &pcd_in)
{
...
bool FastLioSamScQnClass::checkIfKeyframe(const PosePcd &pose_pcd_in, const PosePcd &latest_pose_pcd)
{
  return m_keyframe_thr < (latest_pose_pcd.pose_corrected_eig.block<3, 1>(0, 3) - pose_pcd_in.pose_corrected_eig.block<3, 1>(0, 3)).norm();
}

Eigen::Matrix4d FastLioSamScQnClass::icpKeyToSubkeys(const int &closest_idx, const std::vector<PosePcd> &keyframes, bool &if_converged, double &score)
{
  ...
  src_raw_ = transformPcd(keyframes.back().pcd, keyframes.back().pose_corrected_eig);
...
Eigen::Matrix4d FastLioSamScQnClass::coarseToFineKeyToKey(const int &closest_idx, const std::vector<PosePcd> &keyframes, bool &if_converged, double &score)
{
  Eigen::Matrix4d output_tf_ = Eigen::Matrix4d::Identity();
  if_converged = false;
